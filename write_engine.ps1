...
//    확장: grade × subject × stage — 새 과목/학년은 이 객체에만 추가
// ══════════════════════════════════════════════════════════════════════════════
    ...
    "English": {
      "WA1": {
        ...
        sections: [
          ...
          { id:"C", title:"Comprehension Cloze",   count:8, type:"cloze", hasTrap:true  }, // ✅ fixed
          ...
          { id:"G", title:"Comprehension OEQ",     count:8, type:"open",  hasTrap:true  }, // ✅ fixed
        ],
        topicTags: {
        ...
        trapTypes: {
          ...
          C: ["Conjunction Mismatch (however vs so)","Pronoun Disagreement","Wrong Tense Context"], // ✅ fixed
          ...
          G: ["Answer in Present instead of Past Tense","Lifting text without changing pronouns","Missing 'Why' in Why-question","Inferring beyond given evidence"], // ✅ fixed
        },
      },
      "WA2": {
...
// 리메디에이션 난이도 다운그레이드 규칙 (30년 교육 원칙)
// Hard 섹션 → 취약 개념 리메디에이션은 Medium으로
// Medium 섹션 → 취약 개념 리메디에이션은 Easy로
// Easy 섹션 → Easy 유지 (더 내릴 수 없음)
...
function buildSystemPrompt(context, curriculum, sectionTargets, frequentErrors) {
  ...
  // 각 섹션의 리메디에이션 난이도를 프롬프트에 명시적으로 포함
  const sectionSpec = sections.map(s => {
    ...
    return `  §${s.id} (${s.title}): ${s.count}Q | type=${s.type} | target=${tgt}${s.hasTrap ? " | trap_type required" : ""} | remediation_difficulty=${remDiff}`;
  }).join("\n");

  const topicSpec = Object.entries(topicTags)
    .map(([sec, tags]) => `  §${sec}: [${tags.join(", ")}]`).join("\n");

  const trapSpec = Object.entries(trapTypes)
    .filter(([,v]) => v.length > 0)
    .map(([s, ts]) => `  §${s}: [${ts.join(", ")}]`).join("\n");

  // 오답 패턴 — 이전 함정 구체적으로 명시
  const errorSpec = frequentErrors.length > 0
    ? frequentErrors.map(e => {
        const tgt = sectionTargets[e.section]?.target_difficulty || "Medium";
        const remDiff = getRemediationDifficulty(tgt);
        return `  - §${e.section || "ANY"} | tag="${e.tag}" | errors=${e.count}× | previous_trap="${e.lastTrapType || "unknown"}" | remediation_difficulty=FORCE_${remDiff}`;
      }).join("\n")
    : "  (none)";
...
════════════════════════════════════════════════
§A & §E (MCQ): Exactly 4 options. Distractors must be grammatically plausible, not absurd.
§B (Grammar Cloze): Use ___ for blanks. Test ONE grammar rule per blank clearly.
§C (Comprehension Cloze): Write ONE passage (100–150 words), then 8 fill-in blanks referencing it.
  Traps in §C target discourse connectors and pronoun/tense coherence.
§D (Vocabulary Contextual): Full sentence using target word in natural context.
§F (Editing): One sentence with EXACTLY ONE error. "answer" = full corrected sentence.
§G (Comprehension OEQ): Q1 (G-001) provides the full passage (120–180 words).
  Q2–Q8 are open-ended. Answers must be full sentences, past tense if passage is past tense.
  Traps in §G catch students who answer in wrong tense or lift text verbatim without pronoun change.
...
function buildUserPrompt(context, performance, curriculum, sectionTargets) {
  ...
  // 섹션별 리메디에이션 난이도 계산표 — AI에 명시적으로 전달
...
VERIFY before outputting:
  ...
  □ §G answers should be in past tense if passage uses past tense
  □ §C passage coherence — all blanks flow naturally in context
...
function validateResult(result, curriculum, performance, sectionTargets) {
  ...
  curriculum.sections.forEach(sec => {
    ...
    if (!s) { errors.push(`§${sec.id} missing entirely`); return; }
    const qs = s.questions || [];
    totalQ += qs.length;

    if (qs.length !== sec.count)
      errors.push(`§${sec.id}: expected ${sec.count}Q, got ${qs.length}`);
    ...
    if (isPastPassage) {
      gSec.questions.slice(1).forEach((q, i) => {
        if (q.answer && /\b(is|am|are|has|goes|says)\b/.test(q.answer))
          warnings.push(`§G Q${i+2}: answer uses present tense but passage is past tense`);
      });
    }
  }
...
const DEFAULT_PERF = {
  ...
  // frequentErrors: lastTrapType = 이전 세션에서 걸린 정확한 함정 유형
  frequentErrors: [
    ...
    <div style={{ marginBottom:13 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:4 }}>
        <span style={{ fontSize:12, fontWeight:600, color:"#1a1a2e" }}>
          §{secId} <span style={{ color:"#999", fontWeight:400 }}>— {desc}</span>
        </span>
    ...
    <div style={{
      ...
      <div style={{ flex:1 }}>
        ...
        <div style={{ fontSize:10, color:"#888", marginTop:2 }}>
          §{err.section || "Any"} · {err.count}× errors · Last: Session {err.lastSeen}
        </div>
        {err.lastTrapType && (
...
function QuestionCard({ q, idx, remDiff }) {
  ...
  const slotLabel = { A:"🔁 Remediation", B:"📚 Consolidation", C:"🚀 Stretch" };
  ...
  return (
    <div style={{
      ...
      <button onClick={() => setOpen(o=>!o)} style={{
        ...
        <span style={{ color:"#CCC", fontSize:13, transform:open?"rotate(180deg)":"none", transition:"0.2s", flexShrink:0 }}>▾</span>
      </button>
      {open && (
        <div style={{ padding:"0 12px 12px", borderTop:"1px solid #F0F0F0" }}>
          ...
          {!q.options && q.answer && (
            <div style={{ background:"#E8F5E9", borderRadius:7, padding:"6px 10px", margin:"6px 0" }}>
              <span style={{ fontSize:11, color:"#2E7D32", fontWeight:700 }}>✓ {q.answer}</span>
            </div>
          )}
          {q.explanation && (
            <p style={{ fontSize:11, color:"#555", margin:"6px 0 4px", lineHeight:1.65 }}>💡 <em>{q.explanation}</em></p>
          )}
          {q.memory_hook && (
            <div style={{ background:"#E8EAF6", borderRadius:7, padding:"6px 10px", marginTop:4 }}>
              <span style={{ fontSize:11, color:"#3949AB", fontWeight:700 }}>🧠 {q.memory_hook}</span>
            </div>
          )}
          {q.is_remediation && (
    ...
    <div style={{ border:"1px solid #E0E0EE", borderRadius:14, marginBottom:10, overflow:"hidden", background:"#fff" }}>
      <button onClick={() => setCollapsed(c=>!c)} style={{
        ...
        <div style={{ display:"flex", alignItems:"center", gap:9 }}>
          <span style={{ color:"#fff", fontWeight:900, fontSize:13 }}>§{secDef.id}</span>
          <span style={{ color:"#6670AA", fontSize:11 }}>{secDef.title} · {qs.length}Q</span>
          {remCount > 0 && <Chip label={`🔁 ${remCount}`} bg="#FCE4EC" color="#B71C1C" />}
          {stretchCount > 0 && <Chip label={`🚀 ${stretchCount}`} bg="#E8F5E9" color="#2E7D32" />}
        </div>
        <div style={{ display:"flex", gap:7, alignItems:"center" }}>
          <DiffChip diff={targetDiff} />
          <span style={{ color:"#444", fontSize:12, transform:collapsed?"rotate(-90deg)":"none", transition:"0.2s" }}>▾</span>
        </div>
      </button>
      {!collapsed && (
    ...
    <div style={{ marginBottom:12 }}>
      <div style={{
        ...
        <span style={{ fontSize:12, fontWeight:700, color:errors.length===0?"#2E7D32":"#B71C1C" }}>
          {errors.length===0 ? "✅ Validation passed" : `❌ ${errors.length} error(s) found`}
...
export default function GeniusAdaptiveEngine() {
  ...
  // sectionTargets는 perf + curriculum에서 실시간 계산
  ...
  const TABS = [
    { id:"config",  label:"⚙️ Curriculum" },
    { id:"student", label:"📊 Accuracy" },
    { id:"errors",  label:`⚠️ Error Patterns${perf.frequentErrors.length>0?` (${perf.frequentErrors.length})`:""}` },
                ...
                {curriculum.sections.map(s => {
                  ...
                  return (
                    <div key={s.id} style={{
                      ...
                      <div>
                        <span style={{ fontWeight:800, fontSize:12, color:"#1a1a2e" }}>§{s.id}</span>
        ...
        {activeTab==="student" && curriculum && (
          <div>
                ...
                {curriculum.sections.map(s => {
                  ...
                  return (
                    <div key={s.id} style={{ padding:"5px 11px", borderRadius:9, background:m.bg, textAlign:"center" }}>
                      <div style={{ fontWeight:900, fontSize:13, color:m.text }}>§{s.id}</div>
            ...
            <button onClick={generate} disabled={status==="loading"} style={{
              ...
              {status==="loading" ? "⚙️  Assembling adaptive set…" : "✨  Generate Adaptive Session"}
            </button>
          </div>
        )}
          ...
          <div>
            ...
            <div style={{ background:"#E8EAF6", borderRadius:12, padding:"11px 14px", marginBottom:12, border:"1px solid #C5CAE9" }}>
              <p style={{ fontSize:11, fontWeight:800, color:"#3949AB", margin:"0 0 4px" }}>
                🧠 Remediation Paradox Prevention
              </p>
              <p style={{ fontSize:11, color:"#3949AB", margin:0, lineHeight:1.6 }}>
                When a student fails a concept repeatedly, the AI will <strong>downgrade its difficulty</strong> (Hard→Medium, Medium→Easy) to rebuild understanding.
              ...
              <div style={{ display:"flex", gap:8, marginTop:8, flexWrap:"wrap" }}>
                {[["Hard section","Hard → Medium"],["Medium section","Medium → Easy"],["Easy section","Easy → Easy"]].map(([l,r]) => (
            ...
            <div style={{ background:"#fff", borderRadius:14, padding:16, border:"1px solid #E0E0EE" }}>
              ...
              <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginBottom:8 }}>
                <select value={newSec} onChange={e=>setNewSec(e.target.value)} style={{
                  ...
                  {curriculum.sections.map(s=><option key={s.id} value={s.id}>§{s.id}</option>)}
                </select>
          ...
          <div>
            {status==="idle" && (
              <div style={{ textAlign:"center", padding:"50px 20px", color:"#AAA" }}>
                ...
                <p style={{ fontSize:11 }}>Set accuracy → add error patterns → Generate.</p>
              </div>
            )}
            {status==="loading" && (
              <div style={{ textAlign:"center", padding:"50px 20px" }}>
                <style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>
                <div style={{ fontSize:40, animation:"spin 1.1s linear infinite", display:"inline-block", marginBottom:12 }}>⚙️</div>
                    ...
                    <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
                      {Object.entries(result.slot_allocation).map(([sec, slots]) => (
                        <div key={sec} style={{ fontSize:10, background:"#F8F8FF", borderRadius:7, padding:"4px 9px", border:"1px solid #EEEEF8" }}>
                          <strong>§{sec}</strong>
                          {" "}🔁{slots.slot_a_remediation||0} 📚{slots.slot_b_consolidation||0} 🚀{slots.slot_c_stretch||0}
                        </div>
                      ))}
                    </div>
                  </div>
...
'@
[System.IO.File]::WriteAllText("$env:USERPROFILE\psle-app\src\components\GeniusAdaptiveEngine.jsx", $content, [System.Text.Encoding]::UTF8)
